///
/// Auto-generated python bindings for optimizer: navi_fast
///
...
#[pymodule]
fn navi_fast(_py: Python, m: &PyModule) -> PyResult<()> {
...
impl Solver {
    ...
    fn run(
        ...
        let mut u = [0.0; NAVI_FAST_NUM_DECISION_VARIABLES];

        // ----------------------------------------------------
        // Set initial value
        // ----------------------------------------------------
        if let Some(u0) = initial_guess {
            if u0.len() != NAVI_FAST_NUM_DECISION_VARIABLES {
                println!(
                    "1600 -> Initial guess has incompatible dimensions: {} != {}",
                    u0.len(),
                    NAVI_FAST_NUM_DECISION_VARIABLES
                );
                return Ok(None);
            }
        ...
        // Check parameter
        // ----------------------------------------------------
        if p.len() != NAVI_FAST_NUM_PARAMETERS {
            println!(
                "3003 -> wrong number of parameters: {} != {}",
                p.len(),
                NAVI_FAST_NUM_PARAMETERS
            );
            return Ok(None);
        }
