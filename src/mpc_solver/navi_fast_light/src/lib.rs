...
pub const NAVI_FAST_LIGHT_NUM_DECISION_VARIABLES: usize = 40;

/// Number of parameters
pub const NAVI_FAST_LIGHT_NUM_PARAMETERS: usize = 2778;

/// Number of parameters associated with augmented Lagrangian
pub const NAVI_FAST_LIGHT_N1: usize = 40;

/// Number of penalty constraints
pub const NAVI_FAST_LIGHT_N2: usize = 1;
...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(NAVI_FAST_LIGHT_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, NAVI_FAST_LIGHT_N1, NAVI_FAST_LIGHT_N2)
}
...
fn unscale_result(solver_status: &mut Result<AlmOptimizerStatus, SolverError>) {
    if let Ok(sss) = solver_status {
        let w_cost : f64 = icasadi_navi_fast_light::get_w_cost();
...
pub fn solve(
    ...
    assert_eq!(p.len(), NAVI_FAST_LIGHT_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), NAVI_FAST_LIGHT_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    // Start by initialising the optimiser interface (e.g., set w=1)
    icasadi_navi_fast_light::init_navi_fast_light();
    ...
    if DO_PRECONDITIONING {
        ...
        icasadi_navi_fast_light::precondition(u, p);

        // Compute initial penalty
        icasadi_navi_fast_light::initial_penalty(u, p, & mut rho_init);
    }

    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_navi_fast_light::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_navi_fast_light::grad(u, xi, p, grad);
        Ok(())
    };
    
    let f1 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_navi_fast_light::mapping_f1(u, p, res);
        Ok(())
    };
    let f2 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_navi_fast_light::mapping_f2(u, p, res);
        Ok(())
