...
const NUM_STATIC_PARAMETERS: usize = 2778;
...
extern "C" {
    fn init_interface_navi_fast_light();
    fn cost_function_navi_fast_light(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double) -> c_int;
    fn grad_cost_function_navi_fast_light(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double)
        -> c_int;
    fn mapping_f1_function_navi_fast_light(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double,
    ) -> c_int;
    fn mapping_f2_function_navi_fast_light(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double,
    ) -> c_int;
    // Compute all preconditioning parameters
    fn preconditioning_www_navi_fast_light (
        arg: *const *const c_double,
    ) -> c_int;
    fn init_penalty_function_navi_fast_light(
        arg: *const *const c_double,
        ipnlt: *mut *mut c_double,
    ) -> c_int;
    fn get_w_cost_navi_fast_light() -> c_double;
} // END of extern C
...
pub fn get_w_cost() -> f64 {
    unsafe {
        get_w_cost_navi_fast_light() as f64
    }
}
...
pub fn grad(u: &[f64], xi: &[f64], static_params: &[f64], cost_jacobian: &mut [f64]) -> i32 {
    ...
    unsafe {
        grad_cost_function_navi_fast_light(
            arguments.as_ptr(),
            grad.as_mut_ptr()
        ) as i32
    }
...
pub fn mapping_f1(
    ...
    unsafe {
         mapping_f1_function_navi_fast_light(
            arguments.as_ptr(),
...
pub fn mapping_f2(
    ...
    unsafe {
         mapping_f2_function_navi_fast_light(
            arguments.as_ptr(),
            constraints.as_mut_ptr()
        ) as i32
    }
...
pub fn precondition(
    ...
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    unsafe {
         preconditioning_www_navi_fast_light(arguments.as_ptr()) as i32
    }
}

/// Computes the initial penalty
///
/// Make sure that you have called init_navi_fast_light
pub fn initial_penalty(
    u: &[f64],
    static_params: &[f64],
    rho_init: &mut f64,) -> i32 {

    // preconditioning_init_penalty_function_navi_fast_light
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    let ip = &mut [rho_init as *mut c_double];

    unsafe {
         init_penalty_function_navi_fast_light(
            arguments.as_ptr(),
            ip.as_mut_ptr(),
        ) as i32
    }
